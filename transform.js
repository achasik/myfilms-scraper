...
const getIds = async function(torrent) {
  ...
  if (!torrent.magnet && torrent.url.indexOf("kinozal") > 0) {
    const mlink =
      "http://s-kinozal-tv.appspot.com/getmagnet?" +
      torrent.url.substring(torrent.url.lastIndexOf("?") + 1);
...
const possibleFilm = function(torrent) {
  ...
  if (torrent.url.indexOf("kinozal") == -1) {
    ...
    possible.name =
      names.length > 1 ? sanitize(names[names.length - 1].trim()) : "";
    possible.year = /((19|20)\d{2})/.test(tail(title))
      ? tail(title).match(/((19|20)\d{2})/)[1]
      : 1900;
  } else {
    ...
    if (
      possible.name &&
      (isRu(possible.name) || /^(19|20)\d{2}$/.test(possible.name))
    )
      possible.name = "";
    possible.year = /((19|20)\d{2})/.test(title)
      ? title.match(/((19|20)\d{2})/)[1]
      : 1900;
  }
  return possible;
};
