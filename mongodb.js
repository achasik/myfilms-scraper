...
module.exports = {
    ...
    addTorrent: async(torrent, film)=>{
        const exist = await Torrent.findOne({trackerId: torrent.trackerId, myId: torrent.myId});
        if(exist) {
    ...
    search: async(film)=>{
        var query = {$or:[]};
        if (film.tmdb)
            query.$or.push({tmdb: film.tmdb});
        if (film.kp)
            query.$or.push({kp: film.kp});
        if (film.imdb)
            query.$or.push({imdb: film.imdb});
        if(film.name)
